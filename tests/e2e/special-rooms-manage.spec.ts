 ...
 * 링크를 직접 열어 봐야 알 수 있었고, 오타 하나에 예약표를 새로 만들어야 했다.
 */
...
test('안내 문구를 적지 않아도 예약표가 만들어진다', async ({ page }) => {
  await createBoard(page, { title: '안내 없는 예약표' });

  await expect(page.getByRole('heading', { name: '안내 없는 예약표' })).toBeVisible();
  await expect(page.getByLabel(/^안내 문구/)).toHaveValue('');
});

test('만든 뒤에도 제목과 안내 문구가 화면에 남아 있다', async ({ page }) => {
  await createBoard(page, { title: '컴퓨터실 예약표', description: '사용 후 정리 부탁드립니다' });

  await expect(page.getByLabel('예약표 이름')).toHaveValue('컴퓨터실 예약표');
  await expect(page.getByLabel(/^안내 문구/)).toHaveValue('사용 후 정리 부탁드립니다');
});

test('안내 문구를 고치면 공개 화면에 반영된다', async ({ page }) => {
  await createBoard(page, { title: '컴퓨터실 예약표', description: '처음 적은 안내' });
...
test('안내 문구를 지우면 공개 화면에서도 사라진다', async ({ page }) => {
  await createBoard(page, { title: '컴퓨터실 예약표', description: '지울 안내' });
  const link = await publicLink(page);
...
test('제목을 고치면 머리글과 공개 화면이 함께 바뀐다', async ({ page }) => {
  ...
  await page.getByLabel('예약표 이름').fill('새 이름');
  await page.getByRole('button', { name: '저장', exact: true }).click();
...
test('제목을 비우면 막고 그 칸으로 되돌려 보낸다', async ({ page }) => {
  await createBoard(page, { title: '컴퓨터실 예약표' });

  await page.getByLabel('예약표 이름').fill('   ');
  await page.getByRole('button', { name: '저장', exact: true }).click();

  await expect(page.getByRole('alert')).toContainText('예약표 이름');
  await expect(page.getByLabel('예약표 이름')).toBeFocused();
});
