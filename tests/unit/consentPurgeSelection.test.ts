...
const form = (patch: Partial<ConsentLocalDraft> = {}): ConsentLocalDraft => ({
  ...
  passwordEnabled: false, passwordHash: '', allowResubmission: false, responseCount: 0, status: 'closed',
  closedAt: '2025-01-15T00:00:00.000Z', ...patch,
});
...
describe('보유 기간이 지난 수합 고르기', () => {
  ...
  it('업무 종료일을 읽을 수 없으면 대상으로 삼지 않는다', () => {
    expect(retentionDeadline('언제인지 모름', 12)).toBeNull();
    expect(isPastRetention(form({ closedAt: '언제인지 모름' }), now)).toBe(false);
  });

  it('기한이 지난 것만 고른다', () => {
    const old = form({ id: 'old', closedAt: '2025-01-15T00:00:00.000Z' });
    const recent = form({ id: 'recent', closedAt: '2026-08-01T00:00:00.000Z' });
  ...
  it('보관 개월을 늘리면 대상에서 빠진다', () => {
    const kept = form({ closedAt: '2025-01-15T00:00:00.000Z', retentionMonths: 36 });
    expect(isPastRetention(kept, now)).toBe(false);
  });

  it('진행 중인 업무는 오래됐어도 파기 대상으로 삼지 않는다', () => {
    expect(isPastRetention(form({ status: 'open', closedAt: undefined }), now)).toBe(false);
  });
