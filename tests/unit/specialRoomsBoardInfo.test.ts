...
/**
 * 안내 문구는 비워도 되고, 제목만 있으면 예약판이 성립한다.
 *
...
describe('제목과 안내 문구를 검사한다', () => {
  test('안내 문구는 비워도 저장된다', () => {
    const checked = checkBoardInfo({ title: '컴퓨터실 예약판', description: '' });
  ...
  test('제목은 있어야 한다', () => {
    ...
    expect(checked.error).toContain('예약판 이름');
  });
...
describe('고친 것이 있을 때만 저장한다', () => {
  const saved = { title: '과학실 예약판', description: '정리 부탁드립니다' };
  ...
  test('공백만 더한 것은 바뀐 것이 아니다', () => {
    ...
    expect(boardInfoChanged(saved, { title: '  과학실 예약판  ', description: '정리 부탁드립니다 ' })).toBe(false);
  });
  ...
  test('제목을 고친 것은 바뀐 것이다', () => {
    expect(boardInfoChanged(saved, { ...saved, title: '미술실 예약판' })).toBe(true);
  });
});
