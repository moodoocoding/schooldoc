...
export const parseExcelFile = async (file: File): Promise<ImportedRow[]> => {
  if (file.name.toLocaleLowerCase().endsWith('.csv')) {
    ...
    return new Promise((resolve, reject) => {
      Papa.parse<ImportedRow>(file, {
        ...
        skipEmptyLines: true,
        complete: (result) => {
          if (result.errors.length > 0) reject(new Error(result.errors[0].message));
  ...
  const headers = rows[0].map((cell, index) => String(cell ?? `열 ${index + 1}`).trim());
