/**
 * 쓰이지 않는 화면 시안. 실제 동작은 하나도 없다.
 *
 * 지금은 어디에서도 그려지지 않는다. App.tsx가 만들어진 도구 셋(학생 결과 안내·가정통신문
 * 수합·등록부 서명)만 각자의 경로로 보내고, 이 화면을 여는 activeToolId는 값이 채워지지
 * 않기 때문이다. 유일한 진입점이던 '진행 중인 업무' 목록은 activeTasks가 빈 배열로 고정돼
 * 있어 항목이 생기지 않는다.
 *
 * 원래는 아직 만들지 않은 도구를 누르면 이 화면이 열렸다. 단계 표시와 파일 업로드, 완료
 * 버튼까지 갖춰 동작하는 것처럼 보이지만 올린 파일은 이름만 화면에 띄우고 아무 데도 저장되지
 * 않는다. 교사가 실제 학생 자료를 올리고 처리된 줄 알 수 있어, 카드를 눌러도 열리지 않도록
 * 막았다.
 *
 * 새 도구를 만들 때 이 파일을 그대로 연결하지 말 것. 화면 구성과 단계 흐름을 참고하는
 * 용도로만 두었고, 저장·전송이 붙지 않은 채로 살아나면 같은 함정이 된다.
 */
