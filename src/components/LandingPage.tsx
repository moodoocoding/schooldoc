...
export const LandingPage: React.FC<LandingPageProps> = ({ setActiveTab }) => {
  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      {/* Hero Section - Trustworthy Deep Blue Theme */}
      <section className="bg-gradient-to-r from-blue-950 via-blue-900 to-slate-950 text-white py-20 px-6 sm:px-12 lg:px-24">
        <div className="max-w-6xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-12">
          ...
          <div className="flex-1 text-left space-y-6">
            ...
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight leading-tight">
              학교 행정 업무,<br />
              <span className="text-blue-300">스쿨독과 함께</span> 쉽고 빠르게
            </h1>
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              <button
                ...
                className="bg-white hover:bg-slate-100 text-blue-950 font-extrabold text-xs sm:text-sm px-6 py-3.5 rounded-xl transition shadow"
              >
                📝 행정 도구 시작
              </button>
          ...
          <div className="w-full lg:w-96 bg-white/5 border border-white/10 backdrop-blur-md rounded-3xl p-6 shadow-xl">
            <h4 className="text-xs font-bold text-blue-200 mb-3">빠른 도구 이동</h4>
            <form onSubmit={handleSearchSubmit} className="space-y-3">
              <div className="relative group">
                <input
                  ...
                  className="w-full pl-3 pr-10 py-3 rounded-xl bg-white text-slate-800 placeholder-slate-400 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-blue-400"
                />
              </div>
              <button
                type="submit"
                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs py-3 rounded-xl transition shadow-sm"
              >
                검색하기
              </button>
            </form>
      ...
      <section className="max-w-6xl mx-auto pt-16 pb-20 px-6">
        <div className="text-center mb-12 space-y-3 animate-fade-in">
          <span className="inline-block text-[10px] font-bold bg-blue-50 border border-blue-150 text-blue-600 px-3 py-1 rounded-full uppercase tracking-wider">
            10가지 행정 서비스
          </span>
          ...
          {services.map((service) => {
            ...
            return (
              <div
                ...
                className="bg-white border border-slate-200/60 rounded-3xl p-6 flex flex-col justify-between hover:border-blue-300 hover:shadow-lg transition-all duration-300 cursor-pointer min-h-[220px] group"
              >
                <div>
                  <div className="flex items-center gap-3.5 mb-5">
                    <div className={`w-10 h-10 rounded-xl ${service.bgColor} flex items-center justify-center ${service.accentColor} border border-slate-100/55 flex-shrink-0`}>
                ...
                <div className="text-[11px] font-bold text-blue-600 group-hover:text-blue-800 flex items-center gap-0.5 pt-3.5 border-t border-slate-100 transition-colors">
                  <span>작성하기</span>
                  <span>&rarr;</span>
                </div>
              </div>
