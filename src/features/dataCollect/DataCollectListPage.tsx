...
import { CalendarDays, FileCheck2, Plus, Users } from 'lucide-react';
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6 pb-12">
      <ToolListHeader
        eyebrow="파일을 원본 그대로"
        title="자료 수합"
        description="제목과 안내를 직접 작성하고 확인 또는 수정본을 한곳에서 받습니다."
        toolbar={<ToolHeaderBadge>교사 전용</ToolHeaderBadge>}
        action={<button type="button" onClick={() => navigate('/tools/data-collect/new')} className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-lg bg-[#0F6CBD] px-5 text-sm font-bold text-white hover:bg-[#0B5B9F]"><Plus className="h-4 w-4" />새 자료 수합</button>}
      />
