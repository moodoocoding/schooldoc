...
import { listDataCollections, subscribeDataCollections } from './dataCollectStore';
import { DATA_COLLECTION_KIND_LABELS } from './dataCollectUtils';
...
export function DataCollectListPage() {
  ...
  const [collections, setCollections] = useState<DataCollection[]>(() => listDataCollections(ownerId));
  useEffect(() => {
    const refresh = () => setCollections(listDataCollections(ownerId));
    refresh();
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6 pb-12">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold text-[#0F6CBD]">파일을 원본 그대로</p><h1 className="mt-1 text-2xl font-extrabold sm:text-3xl">자료 수합</h1><p className="mt-2 text-sm text-[#526174]">평가지·계획서·동의서를 배포하고 확인 또는 수정본을 한곳에서 받습니다.</p></div><button type="button" onClick={() => navigate('/tools/data-collect/new')} className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-lg bg-[#0F6CBD] px-5 text-sm font-bold text-white hover:bg-[#0B5B9F]"><Plus className="h-4 w-4" />새 자료 수합</button></div>
