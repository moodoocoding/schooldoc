...
export const createRemoteDataCollection = async (draft: DataCollectionDraft, sourceFile?: File) => {
  ...
  const result = await invoke<DataCollection | { collection: DataCollection }>({ action: 'create', id, title: draft.title, description: draft.description, kind: draft.kind, mode: draft.mode, allowWalkIn: draft.mode === 'custom', dueAt: draft.dueAt, password: draft.password, allowResubmit: draft.allowResubmit, retentionMonths: draft.retentionMonths, targets: draft.mode === 'fixed' ? draft.targets : [], templatePath, templateName: sourceFile?.name ?? '', templateSize: sourceFile?.size ?? 0, templateMime: sourceFile?.type ?? '' });
