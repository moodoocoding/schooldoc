...
export function DataCollectManagePage() {
  ...
  return <div className="mx-auto w-full max-w-7xl space-y-6 pb-12">
    ...
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-bold text-[#0F6CBD]">{DATA_COLLECTION_KIND_LABELS[collection.kind]}</p><h1 className="mt-1 text-2xl font-extrabold sm:text-3xl">{collection.title}</h1><p className="mt-2 max-w-3xl whitespace-pre-wrap text-sm leading-6 text-[#526174]">{collection.description || '별도 안내가 없습니다.'}</p></div><button type="button" onClick={() => void toggleStatus()} className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-lg border border-[#C8D0DA] bg-white px-4 text-sm font-bold text-[#334155]">{collection.status === 'open' ? <PauseCircle className="h-4 w-4" /> : <PlayCircle className="h-4 w-4" />}{collection.status === 'open' ? '수합 종료' : '다시 열기'}</button></div>
