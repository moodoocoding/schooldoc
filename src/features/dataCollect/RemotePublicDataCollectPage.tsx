...
export function RemotePublicDataCollectPage() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try { setWorking(true); setError(''); await submitRemoteDataCollectReview(token, selected.token, effectiveDecision, password, file, note); setDecision(effectiveDecision); setComplete(true); }
  ...
  if (complete) return <RemoteShell><CheckCircle2 className="h-10 w-10 text-[#16803C]" /><h1 className="mt-4 text-xl font-extrabold">회신을 제출했습니다</h1><p className="mt-3 text-sm text-[#526174]">{selected?.label} · {decision === 'confirmed' ? '이상 없음' : '수정본 제출'}</p>{metadata.allowResubmit ? <button type="button" onClick={() => { setComplete(false); setDecision(undefined); setFile(undefined); setNote(''); }} className="mt-6 min-h-[44px] rounded-lg border border-[#0F6CBD] px-5 text-sm font-bold text-[#0F6CBD]">다시 회신하기</button> : <p className="mt-5 text-sm font-semibold text-[#526174]">제출이 끝나 바꿀 수 없습니다.</p>}</RemoteShell>;
