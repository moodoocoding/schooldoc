...
export function StudentResultConfirmDialog({
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') (onDismiss ?? onCancel)();
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-[#0F172A]/45 p-4">
      <div ref={dialogRef} role="alertdialog" aria-modal="true" aria-labelledby="student-result-confirm-title" aria-describedby="student-result-confirm-description" className="w-full max-w-sm rounded-lg border border-[#DCE3EA] bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          ...
          <button type="button" onClick={onDismiss ?? onCancel} className="flex h-10 w-10 items-center justify-center rounded-lg text-[#526174] hover:bg-[#F6F8FB]" aria-label="확인 창 닫기"><X className="h-5 w-5" /></button>
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-2">
          <button ref={cancelRef} type="button" onClick={onCancel} className="min-h-[44px] rounded-lg border border-[#DCE3EA] px-3 text-sm font-bold">{cancelLabel}</button>
