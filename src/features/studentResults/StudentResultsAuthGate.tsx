...
export function StudentResultsAuthGate({ children }: { children: ReactNode }) {
  const { configured, displayName, error, loading, signIn, signOut, user } = useTeacherAuth();

  if (isStudentResultsDemoMode) return children;
    ...
    return (
      <div className="mx-auto grid min-h-[620px] max-w-5xl place-items-center py-10">
        <section className="w-full max-w-md border-y border-[#DCE3EA] bg-white px-6 py-10 sm:px-8">
          <ShieldCheck className="h-8 w-8 text-[#0F6CBD]" />
          <p className="mt-5 text-xs font-bold text-[#0F6CBD]">교직원 전용</p>
          <h1 className="mt-2 text-2xl font-extrabold text-[#0F172A]">학생 결과 안내</h1>
          <p className="mt-3 text-sm leading-6 text-[#526174]">Google 계정으로 로그인해 학생별 결과를 만들고 확인 현황을 관리하세요.</p>
          <button type="button" onClick={() => void signIn('/tools/student-results')} className="mt-7 inline-flex min-h-[48px] w-full items-center justify-center gap-3 rounded-lg border border-[#C8D0DA] bg-white px-5 text-sm font-bold hover:bg-[#F6F8FB]">
            <LogIn className="h-5 w-5 text-[#0F6CBD]" />Google로 로그인
          </button>
