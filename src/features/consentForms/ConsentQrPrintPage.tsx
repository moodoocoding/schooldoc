...
import { RECIPIENTS_PER_SHEET, consentPersonalLink, consentQrSheetFileName, paginateRecipients } from './consentRecipientSheet';
...
export function ConsentQrPrintPage() {
  ...
  const pages = paginateRecipients(recipients);
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      pdf.save(consentQrSheetFileName(draft.title));
    } catch (exportError) {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl pb-12">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 border-b border-[#DCE3EA] pb-4 print:hidden">
        ...
        <button type="button" disabled={exporting || recipients.length === 0} onClick={() => void downloadPdf()} className="inline-flex min-h-[44px] items-center gap-2 rounded-lg bg-[#0F6CBD] px-5 text-sm font-bold text-white disabled:bg-[#AAB7C4]">{exporting ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />}{exporting ? 'PDF 만드는 중' : 'PDF 다운로드'}</button>
      </div>

      <div className="mb-5 print:hidden">
        <h1 className="text-2xl font-extrabold">개인 QR 배부 자료</h1>
        <p className="mt-2 text-sm text-[#526174]">{recipients.length}명 · A4 세로 · 쪽당 {RECIPIENTS_PER_SHEET}명 · 총 {pages.length}쪽</p>
      ...
      {recipients.length === 0
        ? <section className="border-y border-[#DCE3EA] bg-white py-20 text-center print:hidden"><QrCode className="mx-auto h-9 w-9 text-[#94A3B8]" /><h2 className="mt-4 text-lg font-bold">배부할 명단이 없습니다</h2><p className="mt-2 text-sm text-[#526174]">명단 있는 수합으로 만들면 보호자별 QR을 만들 수 있습니다.</p></section>
              ...
              <section key={pageIndex} data-testid="consent-qr-page" className="consent-qr-print-page flex h-[1123px] w-[794px] shrink-0 flex-col bg-white px-[48px] py-[42px] shadow-[0_8px_28px_rgba(15,23,42,0.16)]">
                <header className="mb-5 border-b-2 border-[#0F6CBD] pb-4">
                  <div className="flex items-center justify-between gap-5">
                    <div className="min-w-0"><p className="text-[12px] font-bold text-[#0F6CBD]">가정통신문 수합</p><h2 className="mt-1 truncate text-[21px] font-extrabold text-[#0F172A]">{draft.title}</h2></div>
