...
export function ConsentShareStep({ title, fileName, fieldCount, recipientMode, recipientCount, settings, hasExistingPassword = false, onSettingsChange, onBack, onCreate }: {
  title: string;
  fileName: string;
  fieldCount: number;
  ...
  return <div className="mx-auto w-full max-w-5xl space-y-5 pb-12">
    ...
    <section className="border-y border-[#DCE3EA] bg-white px-4 py-5 sm:px-5"><h2 className="text-sm font-bold">최종 확인</h2><dl className="mt-4 grid gap-x-6 gap-y-4 text-sm sm:grid-cols-2"><div><dt className="text-xs text-[#64748B]">제목</dt><dd className="mt-1 font-bold">{title}</dd></div><div><dt className="text-xs text-[#64748B]">원본 PDF</dt><dd className="mt-1 truncate font-semibold">{fileName}</dd></div><div><dt className="text-xs text-[#64748B]">응답 필드</dt><dd className="mt-1 font-semibold">{fieldCount}개</dd></div><div><dt className="text-xs text-[#64748B]">수합 대상</dt><dd className="mt-1 font-semibold">{recipientMode === 'named' ? `명단 ${recipientCount}명` : '명단 없는 공개 수합'}</dd></div></dl><p className="mt-5 flex items-start gap-2 border-t border-[#EEF1F4] pt-4 text-xs leading-5 text-[#526174]"><LockKeyhole className="mt-0.5 h-4 w-4 shrink-0 text-[#0F6CBD]" />로컬 초안에는 제목과 개수 같은 설정 요약만 저장하며 수신자 이름과 식별값은 저장하지 않습니다.</p></section>
    <div className="sticky bottom-0 z-20 flex items-center justify-between gap-3 border-t border-[#DCE3EA] bg-[#F6F8FB]/95 py-3 backdrop-blur"><button type="button" onClick={onBack} className="min-h-[44px] rounded-lg border border-[#C8D0DA] px-4 text-sm font-bold">이전</button><button type="button" disabled={passwordInvalid} onClick={() => void onCreate()} className="inline-flex min-h-[44px] items-center gap-2 rounded-lg bg-[#0F6CBD] px-5 text-sm font-bold text-white hover:bg-[#0B5B9F] disabled:bg-[#AAB7C4]"><Check className="h-4 w-4" />수합 만들기</button></div>
  </div>;
}
