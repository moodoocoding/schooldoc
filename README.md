# SchoolDoc

SchoolDoc은 교사가 반복해서 처리하는 안내, 수합, 서명, 예약 업무를 한곳에서 관리하기 위한
웹 애플리케이션입니다. 교사용 관리 화면과 학생·참여자용 공개 화면을 분리하고, 모바일 조회와
A4/PDF 출력을 함께 지원하는 것을 목표로 합니다.

## 현재 구현 범위

- 등록부 서명: 참석자 명단 작성·엑셀 가져오기, 공개 서명 링크, 서명 현황, PDF 출력
- 학생 결과 안내: 엑셀 결과 분석, 입력 되돌리기(Ctrl+Z), 학생별 조회·확인·이의, 교사 답변, 개인 QR PDF
- 가정통신문 수합: 원본 PDF에 응답 필드 배치, 공개·개인 링크 응답, 원본 위 응답 합성 PDF
- 자료 수합: 명단 있음·없음 방식, 이름 붙여넣기·Excel 명단 분석, 새 파일 제출 또는 배포 파일의
  `이상 없음` 확인·수정본 회신, 재제출과 제출 현황 관리
- 특별실 예약: 특별실별 공개 예약, 주간 예약 현황과 교사용 관리 화면
- 진행 업무: 종료하지 않은 수합·서명·안내·예약을 도구별 전체 폭 압축 목록으로 모아 보고
  각 도구 목록과 개별 관리 화면으로 이동
- 교사용 Google 로그인과 사용자별 데이터 격리 기반
- 환경 설정: NEIS 소속 학교 검색·계정별 프로필 저장, 20개 색상 테마와 화면 글자 크기 조절

추가 업무 도구는 순차적으로 구현합니다.

## 영수증 AI 분석 연결 규칙

- 기본 화면은 지출 장부(번호·날짜·사용처·사용 내용·금액·영수증)이며, 원본은 지출 행에서 다시 열 수 있어야 한다.
- AI 분석은 사용자에게 OpenAI 전송을 안내한 뒤 `receipt-analyze` 서버 함수에서 수행한다. `OPENAI_API_KEY`는 Supabase Custom secrets에만 등록하고 `VITE_` 변수나 저장소에 넣지 않는다.
- 서버는 Supabase에서 검증한 사용자와 관리자 권한을 확인한다. 클라이언트 개발 모드는 API 호출 권한이 아니다.
- 기본 모델은 `gpt-4.1-mini`, 서버 Secret `RECEIPT_AI_MODEL`로 변경할 수 있다. 모델 변경 후 실제 표본으로 정확도·비용·시간을 재검증한다.
- 파일당 10MB, PDF 10쪽, 파일당 결제 20건으로 제한한다. 기존 `consume_registry_rate_limit` RPC에 독립된 `receipt-ai:` 키를 사용해 사용자별 10회/분·50회/24시간, 전체 100회/24시간을 제한한다. 실패한 시도도 한도에 포함하며, 제한 확인에 실패하면 AI 호출을 중단한다.
- 날짜·사용처·금액이 불확실하면 빈 값으로 검토를 요청한다. 구매 품목에서 교사의 사용 목적을 추측하지 않는다. 합계와 잔액은 AI가 아닌 코드에서 계산한다.
- `store:false`로 OpenAI 응답 저장을 끄되 이를 공급자의 완전한 무보관 보장으로 안내하지 않는다. 요청·영수증·API 키는 로그에 남기지 않는다.
- 원본은 계정·장부별 IndexedDB, 장부와 수정 초안은 localStorage에 보관한다. 기기 간 동기화·서버 백업으로 표현하지 않는다. 이전 버전에서 저장되지 않은 원본은 동일 파일 확인 후 다시 연결한다.
- 서버 함수를 배포할 때는 이름을 지정한다. 운영의 JWT 검증 방식 변경과 외부 전송 활성화는 승인 후 진행하며, 신규 함수 내부 인증을 생략하지 않는다.

## 개발 환경

- React 19, TypeScript, Vite
- Tailwind CSS
- Supabase
- Vitest, Playwright

```bash
npm install
npm run dev
```

프로덕션 빌드와 기본 검증은 다음 명령으로 실행합니다.

```bash
npm run build
npm run typecheck
npm run lint
npm test
npm run test:e2e
```

**타입 검사는 `npm run typecheck`(`tsc -b --noEmit`)로 합니다.** 루트 `tsconfig.json`은
`"files": []`인 참조 전용 설정이라 `npx tsc --noEmit`은 파일을 한 개도 읽지 않고 그냥
통과합니다. 통과했다고 검사된 것이 아닙니다. 실제로 그렇게 확인하다 `booking.bookingDate`
처럼 없는 필드를 읽는 코드가 그대로 배포될 뻔했습니다.

Supabase 연결 정보는 저장소에 커밋하지 않는 `.env.local`에 설정합니다.

```dotenv
VITE_SUPABASE_URL=...
VITE_SUPABASE_PUBLISHABLE_KEY=...
```

## 개인정보 암호화 키

학생 정보와 보호자 명단은 평문으로 저장하지 않고 Edge Function에서만 풀 수 있는
암호문으로 보관합니다. 아래 키를 Supabase에 등록해야 해당 기능이 동작합니다.

| 이름 | 용도 |
|---|---|
| `CONSENT_FORMS_ENCRYPTION_KEY` | 가정통신문 수신자 명단 |
| `STUDENT_RESULTS_ENCRYPTION_KEY` | 학생 결과 안내 개인정보 |
| `REGISTRY_ENCRYPTION_KEY` | 등록부 참석자의 항목 값(소속·직위 등) |
| `DATA_COLLECT_ENCRYPTION_KEY` | 자료 수합 대상 이름·파일명·전달 사항 |

```bash
npx supabase secrets set CONSENT_FORMS_ENCRYPTION_KEY=<64자리 16진수>
```

기능마다 필요한 키가 다르며, 키가 없으면 해당 기능의 암호화 경로가 503으로 중단됩니다.
가정통신문의 명단 없는 공용 링크처럼 키를 쓰지 않는 경로만 예외입니다.

**이 키들은 저장된 개인정보를 푸는 유일한 수단이라 잃어버리면 복구할 수 없고, 교체해도
기존 데이터를 읽지 못합니다.** 한 번 정하면 바꾸지 않으며, 프로젝트를 옮길 때도 같은 값을 씁니다.

등록부는 이름을 평문으로 둡니다. 참석자가 행사장에서 두 글자만 넣어 자기를 찾는 흐름을
지켜야 하는데 암호문에는 부분 검색이 통하지 않기 때문입니다. 대신 이름 옆에 붙어 신원을
좁히는 항목 값만 봉인합니다.
Supabase는 값을 다시 보여주지 않으므로 설정할 때 따로 적어 두어야 합니다.

실제 값은 저장소에 올리지 않는 `SECRETS.local.md`에 적어 관리합니다.
이 파일은 `.gitignore`에 등록되어 있으며, 저장소 밖에도 사본을 두는 편이 안전합니다.

## 배포 순서

**마이그레이션을 먼저 적용하고 함수를 나중에 배포합니다.** 순서가 뒤바뀌면 함수가 아직 없는
컬럼을 읽으려다 실패하고, 화면에는 원인을 알 수 없는 오류만 뜹니다. 실제로 겪은 일입니다.

1. 새 암호화 키가 필요하면 먼저 넣습니다. 키가 없으면 해당 기능이 503으로 접힙니다.
   ```bash
   npx supabase secrets set <이름>=$(openssl rand -hex 32)
   ```
2. `migration list`에서 새 로컬 마이그레이션이 원격에 없을 때만 적용합니다.
   ```bash
   npx supabase migration list --linked
   npx supabase db push
   ```
3. 함수를 배포합니다.
   ```bash
   npx supabase functions deploy <함수 이름>
   ```

컬럼이 생겼는지는 이렇게 확인합니다. `[]`가 나오면 성공, `does not exist`면 아직입니다.

```bash
set -a; . ./.env.local; set +a
curl -s "$VITE_SUPABASE_URL/rest/v1/<테이블>?select=<새 컬럼>&limit=1" -H "apikey: $VITE_SUPABASE_ANON_KEY" -H "Authorization: Bearer $VITE_SUPABASE_ANON_KEY"
```

Edge Function은 코드를 푸시해도 자동으로 올라가지 않습니다. 항상 직접 배포해야 합니다.

### 워크트리를 여럿 쓸 때

`git worktree`로 폴더를 나눠 작업하면 **DB는 하나인데 마이그레이션 폴더는 여럿**이 됩니다.
`supabase db push`는 실행한 폴더의 `supabase/migrations/`만 읽으므로, 다른 폴더의
마이그레이션은 보이지도 적용되지도 않습니다.

여기서 두 가지가 걸립니다.

**하나. 번호가 겹치면 나중 것이 조용히 건너뛰어집니다.** Supabase는 파일 이름 앞의 숫자를
적용 이력의 키로 씁니다. 두 폴더가 같은 날 `202608210001`을 만들면, 먼저 적용된 쪽만
기록되고 나머지는 "이미 적용됨"으로 처리됩니다. 표가 만들어지지 않은 채 함수만 올라갑니다.
**새 마이그레이션을 만들기 전에 다른 폴더의 파일 목록을 확인하세요.**

**둘. 상대가 적용하면 내 쪽 `db push`가 거부됩니다.** 로컬에 없는 마이그레이션이 원격
이력에 있으면 CLI가 밀어내기를 멈춥니다. 이력이 갈라진 채로 밀면 위험하기 때문입니다.
상대의 파일만 가져와 이력을 맞추면 됩니다. 이미 적용된 것이라 다시 실행되지 않습니다.

```bash
git checkout main -- supabase/migrations/<상대_마이그레이션>.sql
npx supabase migration list   # 로컬과 원격이 맞는지 확인
npx supabase db push
```

`migration list`에서 `local`과 `remote` 중 한쪽만 채워진 줄이 있으면 어긋난 것입니다.

확인 프롬프트를 건너뛰려면 `--yes`입니다. `-y`는 없는 플래그입니다.

**셋. 저장소에 올리지 않는 문서는 폴더마다 별개입니다.** 추적하지 않는 파일은 워크트리가
각자 갖습니다. 개발 일지를 폴더마다 따로 쓰면 손으로 합쳐야 하고, 실제로 두 번 어긋났습니다.
그래서 실체를 워크트리 바깥에 두고 각 폴더에서 링크로 봅니다. 새 워크트리를 만들면 한 번만
이어 주면 됩니다.

```bash
ln -s ~/Downloads/vibecoding/schooldoc-docs <새_워크트리>/docs
```

`.git/info/exclude`의 항목이 `/docs/`가 아니라 `/docs`여야 합니다. 끝의 `/`는 디렉터리만
맞추는데 git은 심볼릭 링크를 파일로 보기 때문입니다. `exclude`는 워크트리들이 함께 씁니다.

## 제품 원칙

- 교사가 반복 업무를 빠르고 실수 없이 끝낼 수 있어야 합니다.
- 학생과 참여자는 회원가입 없이 필요한 한 가지 행동에 집중할 수 있어야 합니다.
- 개인정보와 개인 링크는 화면, QR, 파일에서 필요한 범위만 노출합니다.
- 가리는 일은 서버에서 합니다. 화면에서만 덮으면 응답에는 원문이 그대로 남습니다.
- **화면에 QR을 그렸다면 언제나 이미지로 내려받을 수 있어야 합니다.** PDF나 인쇄물은 대체가 되지 못합니다.
  QR 하나만 떼어 안내문·공문에 붙이는 일이 잦고, 화면 캡처로 만든 QR은 인쇄하면 잘 읽히지 않습니다.
- 지웠다고 알리기 전에 실제로 지워졌는지 확인합니다. 저장소 삭제는 조용히 실패할 수 있습니다.
- 모바일 공개 화면과 데스크톱 관리 화면을 각각의 사용 맥락에 맞게 설계합니다.
- 화면 미리보기와 A4/PDF 결과의 정보 구조와 레이아웃을 일치시킵니다.
- 원본 문서 위에 얹는 입력은 문서 내용을 가리지 않습니다. 문서가 이미 말하는 것을 겹쳐 적지 않습니다.
- 접근성, 키보드 탐색, 오류 복구와 빈 상태를 완료 조건에 포함합니다.
- **세로 스크롤은 문서 하나만 맡습니다.** 새 화면도 스스로 스크롤 판을 만들지 않습니다.

### 가정통신문 응답 규칙

- PDF 표시·분석은 공통 로더의 동일 버전 `legacy` 본체와 worker를 함께 사용한다. 모바일 화면 폭 검사와 브라우저 API 호환성 검사는 구분한다.
- 학부모 응답은 모든 원본 페이지가 표시된 후 활성화한다. 표시 실패 시 입력·제출을 중단하고 페이지별 재시도를 제공하며 기존 입력값은 유지한다. 내부 오류 문자열을 사용자에게 그대로 표시하지 않는다.
- 문서 위 입력칸은 원본이 비치는 옅은 음영으로 표시하고, 현재 칸은 테두리와 위치 안내로 강조한다. 표시 크기와 모바일 입력 영역을 분리한다.
- 예/아니오·동의/비동의는 하나의 선택 질문으로 묶는다. 필수 응답은 특정 선택지에 동의할 의무가 아니라 질문에 답할 의무다.
- 단일 선택·복수 선택·독립 확인 항목을 구분하며 질문 설정과 응답 검증은 클라이언트·서버가 같은 규칙을 사용한다.
- 공개 응답은 원본 문서에서 직접 입력하고 전체 확인 후 제출한다. 이전·다음은 원본의 입력 위치를 안내할 뿐 다른 칸을 잠그지 않는다. 필수 누락은 제출 전 확인에서 안내하고 입력값은 이동 중 유지한다.
- 모바일은 원본 위치를 강조한 채 하단 확대 입력을 제공하며 원본 보기로 언제든 돌아갈 수 있다. 입력창을 문서 위의 별도 대형 설문으로 대체하지 않는다.
- 기존 독립 체크박스와 받은 응답을 임의로 그룹화하거나 의미를 바꾸지 않는다. 교사가 선택지를 확인해 질문으로 묶는다.
- 질문 정보는 기존 필드 JSON의 선택 속성으로 보관한다. 선택 규칙 변경을 배포할 때는 공개 응답 서버 함수를 먼저 반영한 후 프런트엔드를 배포한다.
- 결과 화면·표·PDF에서 비동의를 동의로 오해하게 표시하지 않는다.

### 자료 수합 접근 규칙

- 자료 수합의 교사용 화면은 특정 이메일 허용 목록을 두지 않고, 로그인한 모든 교사에게 엽니다.
- 교사용 데이터는 Supabase의 사용자별 권한 정책과 서버의 소유자 검사로 격리합니다. 화면에만
  개발자 조건을 거는 코드는 권한 경계로 사용하지 않습니다.
- 제출자가 여는 공개 링크는 로그인 없이 동작하되, 공개 토큰과 필요한 경우 비밀번호 검증을
  통과한 업무만 보여 줍니다.

### 학급 운영비 영수증 정리 규칙

- 첫 버전은 일반 교사가 자기 학급 운영비를 정리하는 개인 장부입니다. 장부 생성은 제목·학년도·
  학급·전체 예산만 입력하며, 명단·공개 링크·마감일·승인 단계는 넣지 않습니다.
- 파일을 올리면 브라우저에서 바로 분석하고 OCR 원문은 저장하지 않습니다. 날짜·사용처·총액은
  교사가 확인·수정한 뒤 `장부에 반영`해야 예산에 포함되며, 사용 목적은 직접 입력합니다.
- 한 사진에 영수증이 여러 장 있으면 종이 영역을 분리해 각각 검토 후보로 만듭니다. 후보는 한 건씩
  확인·수정하고 모두 반영한 뒤에만 다음 파일을 올릴 수 있습니다.
- 정식 공개 전 운영 미리보기는 관리자 역할 또는 지정된 관리자 이메일 계정에만 열고, 일반 교사는
  홈 카드와 직접 주소 모두에서 실행하지 못하게 합니다.
- 한 파일의 검토가 끝나기 전에는 중복된 파일 선택과 다음 단계 안내를 노출하지 않습니다. 분석
  초안이 없는 예전 파일에는 결과 확인 버튼 대신 원본을 다시 선택하는 복구 행동을 제공합니다.
- 예산 현황은 전체 예산·사용 금액·남은 금액·지출 건수로 설명하고, 분석 전 금액은 사용 금액에
  포함하지 않습니다. 분석 실패가 수기 입력까지 막아서는 안 됩니다.

### 개인정보 보관·파기 규칙

- 보관 기간은 계정의 새 업무 기본값입니다. 기존 업무의 기간을 소급해서 바꾸지 않습니다.
- 진행 중인 업무는 파기 대상으로 올리지 않습니다. 업무를 종료한 시점부터 보관 기간을 계산하고,
  기한이 지난 자료는 환경 설정의 파기 예정 목록에서 교사가 확인합니다.
- 무인 자동 파기를 하지 않습니다. 교사가 대상과 수량을 확인한 뒤 명시적으로 동의해야 영구
  파기를 시작합니다.
- 파기는 저장소 파일을 먼저 지우고 실제 삭제를 확인한 뒤 데이터베이스 행을 지웁니다. 파일 삭제가
  실패하면 데이터베이스 기록을 남겨 재시도할 수 있게 합니다.
- 파기 이력에는 사용자 이름, 업무 제목, 파일명 같은 개인정보를 남기지 않습니다.

## 화면 뼈대 규칙

새 화면도 스스로 스크롤 판을 만들지 않습니다. 세로 스크롤은 문서 하나가 맡고, 한 축만
막을 때는 `overflow-x-hidden`이 아니라 `overflow-x-clip`을 씁니다. 어기면 화면 아래쪽에
설명되지 않는 빈 여백이 생깁니다.

이유와 예외는 [`tests/unit/scrollContainersAreDeliberate.test.ts`](tests/unit/scrollContainersAreDeliberate.test.ts)
맨 위에 적혀 있습니다. 그 테스트가 위 두 패턴을 클래스 이름 단계에서 막고,
[`tests/e2e/app-shell-scroll.spec.ts`](tests/e2e/app-shell-scroll.spec.ts)가 화면마다 맨
아래까지 내려 빈 바닥이 없는지 실제로 잽니다. 새 화면을 만들면 e2e 위쪽 `SHELL_ROUTES`에
한 줄 더합니다.

## 문서 규칙

**README에는 "지금 상태"를 적지 않습니다. 언제나 참인 것만 적습니다.**

이번 배포의 진행 상황, 확인 대기 항목, 마이그레이션 적용 여부처럼 곧 낡을 내용은 개발
일지나 PR 본문에 적습니다. 여러 사람이 같은 파일의 다른 성격 내용을 고치면 병합할 때마다
부딪히고, 낡은 상태가 README에 남으면 읽는 사람이 속습니다.

개발 일지(`docs/development-history.md`)는 저장소에 올리지 않고 워크트리 바깥의
`../schooldoc-docs`에 실체를 두고 각 워크트리에서 심볼릭 링크로 봅니다. 워크트리를 나눠
써도 한 파일에 시간순으로 쌓입니다. Codex 쪽에서 쓴 항목은 제목 끝에 `(codex)`를 붙입니다.

## UX/UI 전문가 페르소나

기능을 설계하거나 화면을 검토할 때 [UX/UI 전문가 페르소나](pro/ux-ui-expert.md)를 기준으로
사용합니다. 이 페르소나는 교사용 업무 도구의 정보 구조, 작업 효율, 오류 예방, 접근성,
반응형 화면과 인쇄 결과를 일관된 순서로 검토하고 실행 가능한 개선안을 제시합니다.

화면 변경은 다음 질문을 우선 확인합니다.

1. 사용자가 지금 해야 할 행동을 바로 알아볼 수 있는가?
2. 가장 자주 쓰는 흐름의 클릭과 입력을 줄였는가?
3. 실수했을 때 원인을 이해하고 복구할 수 있는가?
4. 개인정보가 필요한 사람과 화면에만 보이는가?
5. 모바일, 데스크톱, A4/PDF에서 정보가 잘리거나 겹치지 않는가?
